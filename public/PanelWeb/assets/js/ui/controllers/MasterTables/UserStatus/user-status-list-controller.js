...
// Controlador para la lista de estados de usuario (UserStatus)
class UserStatusListController {
    constructor() {
        console.log('🏗️ Inicializando UserStatusListController');
        this.tableBody = document.getElementById('userStatusTableBody');
        this.stats = {
            total: document.getElementById('totalUserStatuses'),
            ...
            totalFooter: document.getElementById('totalUserStatusesFooter'),
    ...
    initEvents() {
        console.log('🎯 Inicializando eventos del controlador UserStatus');
        ...
        this.tableBody.addEventListener('click', (e) => {
            if (e.target.closest('.edit-user-status-btn')) {
                const btn = e.target.closest('.edit-user-status-btn');
                const id = btn.getAttribute('data-id');
                this.handleEditClick(id);
            } else if (e.target.closest('.delete-user-status-btn')) {
                const btn = e.target.closest('.delete-user-status-btn');
                const id = btn.getAttribute('data-id');
        ...
        console.log('✅ Eventos UserStatus inicializados');
    }
    async load(page = 1) {
        console.log('🔄 Cargando estados de usuario, página:', page);
        const params = {
            page,
        ...
        console.log('📋 Parámetros de búsqueda UserStatus:', params);
        this.setLoading(true);
        try {
            const res = await UserStatusService.getUserStatuses(params);
            console.log('✅ Respuesta de la API UserStatus:', res);
            this.render(res.data);
        } catch (err) {
            console.error('❌ Error cargando datos UserStatus:', err);
            this.tableBody.innerHTML = `<tr><td colspan="5" class="text-center text-danger">${err.message}</td></tr>`;
        }
        this.setLoading(false);
    }
    render(data) {
        console.log('🎨 Renderizando datos UserStatus:', data);
        const items = data.items || [];
        console.log('📋 Items UserStatus a renderizar:', items);
        ...
        console.log('✅ Renderizado UserStatus completado');
    }
    ...
    handleEditClick(id) {
        console.log('🔄 Editando UserStatus con ID:', id);
        
        // Buscar los datos del item en los datos actuales
        const userStatusData = this.currentItems?.find(item => item.id == id);
        
        if (!userStatusData) {
            console.error('❌ No se encontraron datos para el ID:', id);
            GlobalToast.show('Error: No se encontraron datos del estado de usuario', 'error');
            return;
        }
        
        if (window.updateUserStatusController) {
            // Pasar tanto el ID como los datos para evitar llamada a la API
            window.updateUserStatusController.showEditModal(id, userStatusData);
        } else {
            console.error('❌ updateUserStatusController no está disponible');
     ...
     * @param {string} name - Nombre del estado de usuario
     * @param {HTMLElement} button - Botón que disparó la acción
     */
    handleDeleteClick(id, name, button) {
        console.log('🗑️ Eliminando UserStatus con ID:', id, 'Nombre:', name);
        ...
        if (!userStatusData) {
            ...
            if (window.GlobalToast) {
                window.GlobalToast.show('Error: No se encontraron datos del estado de usuario', 'error');
            }
            return;
        }
        
        // Verificar que el controlador de eliminación esté disponible
        if (window.deleteUserStatusController) {
            // Usar el controlador de eliminación con modal de confirmación
            window.deleteUserStatusController.handleDeleteButtonClick(button, userStatusData);
        } else if (window.DeleteUserStatusController) {
            // Crear instancia si no existe
            window.deleteUserStatusController = new window.DeleteUserStatusController();
            window.deleteUserStatusController.handleDeleteButtonClick(button, userStatusData);
        } else {
            console.error('❌ DeleteUserStatusController no está disponible');
            if (window.GlobalToast) {
                window.GlobalToast.show('Error: Controlador de eliminación no disponible', 'error');
            }
        }
    }
}
