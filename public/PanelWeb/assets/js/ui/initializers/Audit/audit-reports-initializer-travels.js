/**
 * Inicializador para la página de Reporte de Viajes
 * SOLO carga componentes visuales y controladores base.
 */
class TravelReportsInitializer {
  static async init() {
    console.log('🚕 Inicializando Reporte de Viajes...');
    ...
    try {
      ...
      await loader.loadComponent('topbar', '#topbar-container', {
        pageTitle: window.pageConfig?.pageTitle || { icon: 'fas fa-taxi', text: 'Reporte de Viajes' }
      });
    ...
    setTimeout(() => {
      ...
      this._initTooltips();
      this._initShortcuts();
      console.log('✅ Página de Reporte de Viajes inicializada');
      LoadingScreenUtil.notifyModuleLoaded('TravelReports');
    }, 500);
  }
  ...
  static _initGlobalEvents() {
    ...
    window.addEventListener('error', event => {
      if (event.filename && event.filename.includes('travel')) {
        console.error('❌ Error en módulo de viajes:', event.error);
        window.showToast?.('Error inesperado en reportes de viajes', 'error');
      }
    });
  ...
  static cleanupTravelReportsModule() {
    console.log('🧹 Limpiando módulo de viajes...');
...
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => TravelReportsInitializer.init(), 500);
});

// Limpiar antes de salir
window.addEventListener('beforeunload', () => {
  TravelReportsInitializer.cleanupTravelReportsModule();
});

console.log('📝 TravelReportsInitializer cargado y listo');
